...
Compose(
    ...
    <function _convert_image_to_rgb at 0x13713f790>
    ToTensor()
...
class CLIP_Adapter(nn.Module):
    ...
    def forward(self,out,c):
        ...
        # map_feat -> [1, 768, 7, 7]
        return self.conv(fuse_feat+0.1*map_feat) # [1, 512, 7, 7]


class NetG(nn.Module):
    def __init__(self, imsize, CLIP, ngf=64, nz=100, cond_dim=512, ch_size=3, mixed_precision=False):
    ...
    def forward(self, noise, c, eval=False): # x=noise, c=ent_emb
        with torch.cuda.amp.autocast() if self.mixed_precision and not eval else dummy_context_mgr() as mp:
            cond = torch.cat((noise, c), dim=1) # 612 dim, 100 + 512
...
class NetD(nn.Module):
    def __init__(self, imsize, ndf=64, ch_size=3, mixed_precision=False):
...
class NetC(nn.Module):
    def __init__(self, ndf=64, cond_dim=512, mixed_precision=False):
